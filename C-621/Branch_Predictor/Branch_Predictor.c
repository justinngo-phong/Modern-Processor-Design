...
#ifdef GSHARE
	// Get masked global history by masking it with the global_history_mask
	unsigned masked_global_history = branch_predictor->global_history & branch_predictor->global_history_mask;

	// Get masked branch address by masking it with the global_history_mask. This will ensure the final gShare_idx is not out of bound of the global_counters array.
	unsigned masked_branch_address = branch_address & branch_predictor->global_history_mask;

	// Find the gShare_idx by XORing the two previous values.
	unsigned gShare_idx = masked_global_history ^ masked_branch_address;

	// Get the gShare_prediction direction
	bool gShare_prediction = getPrediction(&(branch_predictor->global_counters[gShare_idx]));

	// Check if the prediction is correct or not.
	bool prediction_correct = gShare_prediction == instr->taken;

	if (instr->taken) {
		incrementCounter(&(branch_predictor->global_counters[gShare_idx]));
	} else {
		decrementCounter(&(branch_predictor->global_counters[gShare_idx]));
	}
